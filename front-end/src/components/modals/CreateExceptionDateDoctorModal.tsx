...
const CreateExceptionDateDoctorModal: FC<CreateExceptionDateModalProps> = ({
    onSuccess
}) => {
    ...
    const onFinish: FormProps<ExceptionDateRequest>['onFinish'] = async (values) => {
        ...
        setLoading(true)
        const response = await settingService.createExceptionDateForDoctor(payload);
        setLoading(false);
                ...
                >
                    <Select mode="multiple" placeholder="Chọn các ngày vắng trong tuần">
                        {weekDaysData.map((weekDay) => (
                            <Select.Option key={weekDay.id} value={weekDay.dayOfWeek}>
                                {weekDay.title}
                            </Select.Option>
                        ))}
                    </Select>
...
export default CreateExceptionDateDoctorModal;
