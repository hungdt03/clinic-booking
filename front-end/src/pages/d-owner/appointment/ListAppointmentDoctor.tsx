...
const ListAppointmentDoctor: FC = () => {
    ...
    const fetchAppointments = async (query: QueryParams) => {
        const response = await appointmentService.getAllAppointmentsByLoggedInDoctorOwner(query);
        if(response.success) {
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    ...
                    <Link to={`/admin/appointment/doctor/${record.id}`}><Button icon={<EyeOutlined />} type="default" size="small">Chi tiết</Button></Link>
                </Space>
            ),
        },
