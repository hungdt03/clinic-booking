import { Button, Input, Popconfirm, Space, Table, TableProps, message } from "antd";
...
export const CancelBox: FC<CancelBoxProps> = ({
    error,
    onChange
}) => {
    return <div className="w-[300px]">
        <TextArea onChange={e => onChange(e.target.value)} placeholder="Lí do hủy" rows={4} />
        {error && <p className="text-sm mt-2 text-red-600">{error}</p>}
    </div>
}


const ListAppointmentClinic: FC = () => {
    ...
    const fetchAppointments = async (query: QueryParams) => {
        const response = await appointmentService.getAllAppointmentsByLoggedInClinic(query);
        if(response.success) {
...
export default ListAppointmentClinic;
