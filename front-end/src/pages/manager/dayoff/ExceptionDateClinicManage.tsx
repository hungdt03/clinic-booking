...
import CreateExceptionDateModal from "../../../components/modals/CreateExceptionDateClinicModal";
import { ExceptionDateEnum } from "../../../enums/ExceptionDateType";

const ExceptionDateManage: FC = () => {
    ...
    const fetchExceptionDates = async () => {
        const response = await settingService.getAllExceptionDatesByClinic();
        if(response.success) {
    ...
    const confirmRemove = async (id: number | string) => {
        const response = await settingService.removeExceptionDateClinicById(id);
...
export default ExceptionDateManage;
