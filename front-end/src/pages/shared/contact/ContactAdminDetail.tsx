    ...
    return <div className="flex flex-col h-full relative">
        ...
        <div className="flex flex-col gap-y-3 px-2 py-4 h-full overflow-y-auto scrollbar-w-2 mb-16 scrollbar-h-4 custom-scrollbar">
            {messages.map(message => {
                if(!message.isVisibleToRecipient || !message.isVisibleToSender)
                    return <Fragment key={message.id}></Fragment>
                return <Message key={message.id} isMe={message.recipient.id == recipientId} message={message} />
            })}
...
export default ContactAdminDetail;
