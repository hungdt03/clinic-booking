...
const ListSpecialityExaminateDoctor: FC = () => {
    ...
    const fetchSpecialities = async () => {
        const response = await specialityService.getAllSpecialitiesByLoggedInDoctor();
    ...
    const handleDeleteSpeciality = async (id: number) => {
        const response = await specialityService.deleteForDoctor(id);
        if(response.success) {
    ...
    const handleSubmit = async (values: SpecialityExaminateRequest) => {
        const response = await specialityService.addSpecialityForDoctor(values);
        if(response.success) {
            message.success(response.message);
            fetchSpecialities()
            handleOk()
...
export default ListSpecialityExaminateDoctor;
